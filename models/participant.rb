...
class Participant
  ...
  def self.connections
    Participant.all.map { |p| {name: p.username, size: p.score, connect_from: connects(p.id)} }
  end

  def self.connects(id)
    connects = []
    p = Participant.find_by_id(id)
    p.connected_to_ids.each do |v|
      puts v
      puts Participant.find_by_id(v)
      connects << Participant.find_by_id(v).username
    end
    connects
  end
end